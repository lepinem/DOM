...
for (let i = 0; i < formData.length; i++) {
  ...
  function renderOptions() {
    // let options = document.getElementsByTagName('select');
    // let optionsInfo = document.createElement('option');
    option.text = formData[i].options[j];
    options.add(optionsInfo);
    optionsInfo.setAttribute('label', formData[i].options.label);
    optionsInfo.setAttribute('name', formData[i].options.name);
    return optionsInfo
  }
  function renderText() {
